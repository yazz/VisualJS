  ...
  (let [coils-trigger      (korma.core/exec-raw
        ...
        (cond
          ...
          (str "create or replace trigger I" table-name " AFTER INSERT ON " table-name " "
                                " FOR EACH ROW "
                                   "BEGIN "
                                   "INSERT INTO coils_realtime_log  (record_timestamp, record_table_name, record_id, record_id_type, record_operation) "
                                   "values \n"
...
(defn make-triggers-table []
  (let [sql-to-create-triggers-table
        (cond
          ...
          CONSTRAINT coils_triggers_pkey PRIMARY KEY (id)
          );
          "
...
(defn make-todo-table []
  (let [sql-to-create-todo-table
        (cond
          ...
          CONSTRAINT coils_todo_items_pkey PRIMARY KEY (id)
          );"
...
(defn make-users-table []
  (let [sql-to-create-users-table
        (cond
          ...
          CONSTRAINT coils_users_pkey PRIMARY KEY (id)
          );
          "
...
(defn make-applications-table []
  (let [sql-to-create-applications-table
        (cond
          (= *database-type* "postgres" )
          "
          CREATE TABLE coils_applications
          (
          id                   serial NOT NULL,
          application_name     character varying,
          application_code     character varying,
          CONSTRAINT coils_applications_users_pkey PRIMARY KEY (id)
          );
          "
          ...
          CONSTRAINT  COILS_APPLICATIONS_PK PRIMARY KEY (ID)
          )")
        ]

    (if (not (does-table-exist  "coils_applications"))
...
(defn make-figwheel-table []
  (let [sql-to-create-figwheel-processes-table
        (cond
          ...
          CONSTRAINT  COILS_FIGWHEEL_PROCESSES_PK PRIMARY KEY (ID)
          )")
        ]

    (if (not (does-table-exist  "coils_figwheel_processes"))
...
(defn make-log-table []
  (let [sql-to-create-admin-table
        (cond
          ...
          CONSTRAINT coils_realtime_log_pkey PRIMARY KEY (id)
          );
          "
          (= *database-type* "oracle" )
          "
...
(defn make-log-table-insert-trigger-function []
  (let [coils-trigger-fn-exists-result      (korma.core/exec-raw
        ...
        (record_timestamp,  record_table_name,  record_id, record_id_type, record_operation)
        VALUES
