...
Then, in early 2014 Zubair listened to a Javascript Jabber postcast with Pete Hunt of Facebook, and learnt about Facebook React and how Facebook used it for building reactive UIs. Then he discovered a ClojureScript library called Om, made by David Nolen (https://github.com/swannodette/om) which provided a Clojurescript wrapper on top of Facebook React. He wanted to use React/Om in Coils, but this was a breaking change for all previous Coils applications (such as NemCV) which used imperative UI libraries like Domina. However, Zubair decided to make the split, so that all new Coils webapps would be built in a reactive way using React/Om.
