...
Then in early 2014 he also discovered Facebook React and incorporated a ClojureScript library called Om, made by David Nolen (https://github.com/swannodette/om) which provided a Reactive UI. He wanted to use this Facebook React library in Coils, but this was a breaking change for all previous Coils applications (such as NemCV) which used imperative UI libraries like Domina. However, Zubair decided to make the split, so the new Coils would use Om and a reactive UI so all future Coils UIs will be built in a reactive way.
