...
Then in early 2014 he also discovered Facebook React for making Reactive UIs and a ClojureScript library called Om, made by David Nolen (https://github.com/swannodette/om) which provided a Clojurescript wrapper on top of Facebook React. He wanted to use this React/Om in Coils, but this was a breaking change for all previous Coils applications (such as NemCV) which used imperative UI libraries like Domina. However, Zubair decided to make the split, so the new Coils would use React/Om and a reactive UI so that all future Coils UIs could be built in a reactive way.
