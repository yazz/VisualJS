...
Wow! Suddenly the world opened up and Zubair realised that there was a better way to develop web software by using interactive languages such as Ruby and easy database access via ActiveRecord. Soon however, he soon became frustrated again, as with Rails everything was generated on the server and control of the web page on the client was not fine-grained enough. So, in true developer style, Zubair did the "wrong thing", and took the long and painful route of trying to find his programming Nirvana, going to Erlang, Vaadin, Grails, JQuery, Javascript, GWT, Google Closure, Dart, and finally deciding to use Clojure and ClojureScript for a project he was involved with.

That project was NemCV, and it is the CV system for Denmark. Seeing that there was alot of good work in NemCV that could be reused Zubair took inspiration from Ruby on Rails, which coincidently came from Denmark too, where a developer called David Heinemeier Hansson made a project called BaseCamp for his company 37 Signals, and then he extracted the Ruby on Rails framework from this product. So based on the Clojurescript work Zubair had done with NemCV he extracted the main parts to build some extra applications needed by NemCV. That extraction of code became the Coils framework.

After around a year of development of Coils against relational databases (as NemCV used Postgres), Zubair met Peter Neubauer from Neo4j when he came to do a talk about Neo4j at MatchingHeads, a Danish startup organisation (http://www.meetup.com/copenhagen-it-people/events/127072702/). Zubair realised that Neo4j could make developing web applications simpler as it allows schema-less development and had a powerful SQL like language called Cypher to query the data store. So Coils has first class support for Neo4j, and uses it as the default backend. So, for example, when end user sessions are recorded in Coils, these are stored in Neo4j.

In early 2014 Zubair listened to a Javascript Jabber postcast with Pete Hunt of Facebook, and learnt about Facebook React and how Facebook used it for building reactive UIs. Then he discovered a ClojureScript library called Om, made by David Nolen (https://github.com/swannodette/om) which provided a Clojurescript wrapper on top of Facebook React. He wanted to use React/Om in Coils, but this was a breaking change for all previous Coils applications (such as NemCV) which used imperative UI libraries like Domina. However, Zubair decided to make the split, so the new Coils would use React/Om so that all future Coils UIs could be built in a reactive way.
...
---------------------------
Because Coils is based around a principle of being a **Maintenance first** framework, this makes Coil not suitable for quite a wide variety of web projects. this is because Coils uses a langauge called Clojurescript, which compiles down to Javascript, which is unsuitable for most developers, who **hate Lisp**, since Clojure is a Lisp. So why use a Lisp language then, if it is so unpopular? Well, Coils does not use Clojurescript for the sake of it. Since Clojurescript is a Lisp it has a feature called Macros which make the time travelling debugger and runtime code inspection possible, which would not be possible in a non-Lisp language. So there is a larger learning curve with Coils, but maintenance time is reduced. So for a variety of use cases the following may be a better choice:
