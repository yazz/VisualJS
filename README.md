...
## A Low Code tool for building small web applications on your local network
...
Visual Javascript is a Low Code tool for building web applications on your local network. It runs on PC/Mac using Docker or NodeJS, or on a Linux/Windows server with Docker, Kubernetes or NodeJS. To develop an app, you simply start Visual Javascript, create the app in your web browser, and share the app with others by sending them the app URL. NodeJS is perfect for small web apps or microservices, but not suitable for high performance apps or for build low level systems. Many demo apps can be found at https://yazz.com (Yazz sponsors development of the tool).
