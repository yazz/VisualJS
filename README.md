...
After around a year of development of Coils against relational databases (as NemCV used Postgres), Zubair met Peter Neubauer from Neo4j when Peter came to do a talk at MatchingHeads, a Danish startup organisation (http://www.meetup.com/copenhagen-it-people/events/127072702/). He realised that Neo4j could make developing applications simpler as it allows schema-less development and had a powerful SQL like language called Cypher to query the graph. So Coils has first class support for Neo4j, and uses it as the default backend. For example, when user sessions are stored in the backend, these are stored in Neo4j.

Then in early 2014 he also discovered Facebook React for making Reactive UIs and a ClojureScript library called Om, made by David Nolen (https://github.com/swannodette/om) which provided a Clojurescript wrapper on top of Facebook React. He wanted to use React/Om in Coils, but this was a breaking change for all previous Coils applications (such as NemCV) which used imperative UI libraries like Domina. However, Zubair decided to make the split, so the new Coils would use React/Om and a reactive UI so that all future Coils UIs could be built in a reactive way.
